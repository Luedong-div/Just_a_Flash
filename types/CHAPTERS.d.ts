...
/** 卡牌 ID 更新时间 2026/2/23 */
type CardName = CardNames[number];
/** 卡牌花色 */
type Suit = "heart" | "diamond" | "club" | "spade" | "none" | null;
/** 卡牌点数 */
...
/** 单张卡牌 */
type Card = CardObject | CardTuple;
/** 卡槽集合：每项为若干张卡片 */
type CardZones = {
	/** 手牌区 */
	h: Card[];
...
export type ConvoEntry = {
	 ...
	 */
	playConvoBefore?: string[][];
	/**
	 * 结束后对白，二维字符串数组
	 * 每项为一个字符串数组（例如 ["说话者id", "台词"] 无 id 则为旁白）
	 */
	playConvoAfter?: string[][];
	[key: string]: unknown;
};

/** 单个难度分支的开局数据，位于 difficulty 的某个数字 key 下 */
export interface LevelDesignUnit {
	/** boss 信息 */
	bossInfo?: Partial<UnitInfo>;
	/** 玩家信息 */
	meInfo?: Partial<UnitInfo>;
	/** 关卡级别的全局配置 */
	global?: LevelGlobal;
	[key: string]: any;
}
...
export interface GameDataEntry {
	/** 对话数据，保存在gameData/CHAPTERS中，自动读取 */
	convo?: ConvoEntry | Record<string, unknown> | null;
	/** 多难度分支的开局设计 */
	difficulty?: LevelDesignEntry;
	[key: string]: any;
}
...
export interface LevelConfig {
	/**
	 * 关卡 id（可省略，运行时会自动生成）
	 * 格式为 c-x-l-y，其中 x 是章节序号，y 是关卡序号，例如 c1l1 表示第一章的第一关
	 */
	id?: string;
	/** 关卡标题，保存在gameData/CHAPTERS中，自动读取 */
	title: string;
	/** 关卡简介，保存在gameData/CHAPTERS中，自动读取 */
	intro?: string;
	/** 关卡提示，保存在gameData/CHAPTERS中，自动读取 */
	hint?: string;
	/** 关卡图片路径（可省略，运行时按 id 自动映射） */
	image?: string | null;
	/** 关卡架构数据 */
	gameData?: GameDataEntry | Record<string, unknown> | null;
	/** 关卡通关奖励（例如 currency/items） */
	reward?: {
		/** 货币奖励 */
...
export interface ChapterConfig {
	/**
	 * 章节 id（可省略，运行时会自动生成）
	 * 格式为 chapter-x，其中 x 是章节序号，例如 chapter1 表示第一章
	 */
	id?: string;
	/** 章节标题 */
	title: string;
	/** 章节关卡列表（对象，键为关卡ID） */
	levels: { [levelId: string]: LevelConfig };
}

export type ChaptersConfig = { [chapterId: string]: ChapterConfig };

/**
 * 章节配置对象，键为章节ID（c1, c2, ...），包含每个章节的标题和关卡信息。每个关卡可以包含标题、简介、提示和图片路径等信息。
 */
declare const CHAPTERS: ChaptersConfig;
